...
export function remapGrid(grid: Grid, from: string[], to: string[]): Grid {
  return grid.map((row) =>
    row.map((cell) => (cell ? { ...cell, color: remapColor(cell.color, from, to) } : null)),
  )
}

export function remapPiece(piece: Piece, from: string[], to: string[]): Piece {
  if (piece.skin !== 'neon') return piece
  return { ...piece, color: remapColor(piece.color, from, to) }
}

export function nextMapOnPerfectClear(map: ArcadeStage, score: number): ArcadeStage | null {
  const earned = stageForScore(score)
  if (earned.level <= map.level) return null
  return nextStage(map)
}
