...
export default function BottomNav({ tab, onChange }: Props) {
  ...
  return (
    <nav className="nav">
      <button className={`nav-btn ${tab === 'play' ? 'active' : ''}`} onClick={() => go('play')}>
      ...
      <button className={`nav-btn ${tab === 'ranks' ? 'active' : ''}`} onClick={() => go('ranks')}>
        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8">
      ...
      <button className={`nav-btn ${tab === 'crate' ? 'active' : ''}`} onClick={() => go('crate')}>
        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8">
